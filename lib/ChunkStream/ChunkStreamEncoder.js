...
const createTimestamp = () => {
	...
	return () => {
		...
		if (timestamp < TIMESTAMP_WRAP) {
			return timestamp;
		}
		else {
			const next = timestamp - TIMESTAMP_WRAP;
...
const encodeMessageHeader = (format, timestamp, messageBodyLength, messageTypeId, messageStreamId) => {
	const timestampSize = timestamp >= TIMESTAMP_MAX ? 4 : 0;
	const buf = Buffer.allocUnsafe(getMessageHeaderSize(format) + timestampSize);
	if (timestampSize) {
		buf.writeUInt32BE(timestamp, buf.length - timestampSize);
	}
	...
	buf.writeUIntBE(Math.min(timestamp, TIMESTAMP_MAX), 0, 3);
	if (format === TYPE_2) {
		return buf;
	}
