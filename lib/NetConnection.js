...
class NetConnection {
	constructor () {
		...
		this[PING_PONG] = new PingPong(NetConnection[PING_FREQUENCY], NetConnection[PING_TIMEOUT]);
		this[SHAREDOBJECT] = [];
	}
	...
	connect (uri, ...args) {
		...
		const onClosedBound = this[onClosed].bind(this);
		...
		client.once('close', onClosedBound);
		client.connect().catch(() => {
			client.off('close', onClosedBound);
		});
	...
	async [onCommand] (client, name, transactionId, command, ...args) {
		if (typeof this[name] !== 'function' || hasOwn.call(Object.getPrototypeOf(this), name)) {
			if (!transactionId) {
				return;
			}
	...
	[APPLY_SO] (name, events, version) {
		...
		client.applySharedObjectEvents(name, events, version);
	}
}

NetConnection[PING_FREQUENCY] = 10000;
NetConnection[PING_TIMEOUT] = 5000;
