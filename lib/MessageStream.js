...
class MessageStream extends Duplex {
	...
	ping () {
		...
		buf.writeUInt16BE(PING_REQUEST);
	...
	sendSharedObject (name, version, flags, ...events) {
		const data = [
		...
		for (let i = 0; i < events.length; i++) {
			data.push(
				Buffer.from([events[i++]]),
				createU32(events[i].length),
				events[i],
			);
		}
		
		this.send(SHARED_OBJECT_MESSAGE_AMF0, Buffer.concat(data));
	}
	...
	_write ({ messageTypeId, messageBody }, encoding, callback) {
		switch (messageTypeId) {
			...
			case USER_CONTROL_MESSAGE: {
				...
				switch (eventTypeId) {
					case PING_REQUEST: {
						const timestamp = messageBody.readUInt32BE(2);
						this.emit('ping', timestamp);
						this.pong(timestamp);
						break;
					}
					case PING_RESPONSE:
						this.emit('pong', messageBody.readUInt32BE(2));
						break;
					default:
						break;
				}
